...
from deepspeed.moe.sharded_moe import TopKGate, _AllToAll, einsum
...
class BRTMOELayer(Base):
    ...
    def forward(self, *input: torch.Tensor, **kwargs: typing.Any) -> torch.Tensor:
        ...
        combined_output = einsum(
            "sec,ecm->sm", combine_weights.type_as(expert_output), expert_output
        )
...
class BRTMOE(nn.Module):
    def __init__(
        ...
        self.deepspeed_moe = BRTMOELayer(
            TopKGate(
                hidden_size,
                num_experts,
                k,
