...
class BRTFusedExperts(nn.Module):
    def __init__(self, expert, capacities, num_local_experts=1):
        ...
        wo_denses = nn.ModuleList(
            [
                nn.Linear(
                    intermediate_size, hidden_size, bias=True, dtype=torch.float16
                )
                for _ in range(num_local_experts)
            ]
        )
        ...
        sample_inputs = [
            torch.randn((i, hidden_size), device="meta", dtype=torch.float16)
            for i in capacities
        ]
        self.fused_wi = make_jit_kernel(
            nn.ModuleList(wi_denses), sample_inputs, "forward", opt_level="homo_fuse"
        )
        sample_inputs = [
            torch.randn((i, intermediate_size), device="meta", dtype=torch.float16)
            for i in capacities
        ]
        self.fused_wo = make_jit_kernel(
            nn.ModuleList(wo_denses), sample_inputs, "forward", opt_level="homo_fuse"
        )

    def forward(self, inputs: torch.Tensor):
        torch.cuda.current_stream().synchronize()
        origin_shape = inputs.shape
        branch_capacities = torch.tensor(
            [inputs.shape[2] for _ in range(self.num_local_experts)],
            device="cuda",
            dtype=torch.int32,
        )
        inputs = torch.flatten(inputs, start_dim=0, end_dim=2)
        wi_out = torch.empty(
            (inputs.shape[0], self.intermediate_size),
        ...
        wo_out = torch.empty(
            (inputs.shape[0], self.hidden_size),
            device=inputs.device,
            dtype=torch.float16,
        )
        torch.cuda.current_stream().synchronize()
